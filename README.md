...
目前串接的表單是 **Jofit 模擬表單**（測試用）。之後要換成正式表單時，把 `backend/api/app/google_form.py` 裡的 `VIEWFORM_URL` 換掉，然後**務必重新跑一次** `backend/api/tests/diagnose_browser.py`（見下面說明）確認欄位順序、送出按鈕文字跟這個表單一致，因為這個機制是靠「頁面上第幾個文字欄位」而不是 `entry.*` ID 在對應問題。
...
- **紀錄／伺服器兩頁**：後端把所有人的預約放在同一張表。「紀錄」分頁只顯示自己的（用設定頁的員工編號比對）；只有林晏瑜看得到的「伺服器」分頁顯示全部，每筆多一行預約人（姓名・員編）。
- **預約紀錄**：排程中放最上面（送出時間相同的合併成一組），其次送出失敗，已送出的依月份摺疊；狀態用膠囊標籤（排程中＝金、已送出＝淺底＋褐色勾、失敗＝磚紅）。
- **送出結果推播**：後端實際送出的那一刻（不管是馬上送出還是排程時間到了才送出），會透過 APNs 推播真的通知到手機告訴你成功或失敗——就算 App 沒開、甚至被滑掉也看得到，因為送出這件事本來就不需要手機在場。
- **管理功能（只有姓名為「林晏瑜」的使用者看得到）**：
  - **多選取消**：「紀錄」和「伺服器」兩頁右上角都有「選取」，勾選排程中的預約（可「全選」＝一鍵全取消；在「紀錄」只涵蓋自己的，在「伺服器」涵蓋所有人的），按「取消所選」後要輸入管理密碼。送出中／已送出的取消不了（Google 表單沒有收回機制），不能勾選。
  - **編輯課程分頁**：新增、刪除、修改課程的名稱／星期／時間，按「儲存變更」並輸入管理密碼後，存到後端，所有人的課表都會更新。如果改動的課程已有排程中的預約，儲存前會警告：這些預約不會跟著更新，仍以建立當時的名稱和時間送出。
  - 這個「只有林晏瑜」是 App 端的軟性開關（姓名是自己在設定頁輸入的）；真正的保護是**後端驗證的管理密碼**（`ADMIN_PIN`，只存在 VPS 的 `.env`，不在 App 或 repo 裡），連續輸錯 5 次會鎖 15 分鐘。
- **首頁小工具**：長按主畫面 →「＋」→ 搜尋「Jofit」加入。當天有**自己**已送出的課程（依員工編號，不含別人的）就顯示日期＋課程名稱＋運動圖案；沒有就顯示日期＋「今天休息」＋休息圖案。
- **避免重複送出**：每人每堂課只會建立一筆預約（一堂課可以有很多學生，各自一筆）、只會真正送出一次，細節見下面「避免重複送出」章節。
...
Jofit/                       # 主 App target
  ...
  Services/CourseStore.swift             # 抓課表（後端有管理員編輯過的就用後端的，否則 courses.json）+ 本地快取 + 換算成未來 4 週的日期
  Views/EditCoursesView.swift            # 管理員專用的「編輯」分頁（見上面「管理功能」）
  Services/ReservationStore.swift        # 同步後端狀態、本地快取（離線用）、同步小工具
  Services/UserSettings.swift            # 姓名/員工編號 設定
  ThemeApp.swift              # App 專用的主題部分：課程類別→色條對照（改關鍵字/顏色在這）、主按鈕樣式、導覽列外觀
  Views/                      # 課程／紀錄／設定 三個分頁 + 篩選 sheet + 首次使用引導畫面（Chips.swift 放日期/星期 chip、狀態標籤）
...
> **兩個來源，後端優先**：管理員在 App 的「編輯」分頁存過課表之後，課表就以後端資料庫裡那份為準，App 不再讀下面的 `courses.json`（它只剩「後端還沒存過課表」時的初始資料／備援）。所以存過之後再去改 `courses.json` 不會有任何效果，要改請用 App 的「編輯」分頁。後端連不上時，App 顯示上次成功抓到的快取，不會退回去讀 `courses.json`（避免拿到比較舊的那份）。
...
2. 後端有一個每 30 秒醒來一次的排程迴圈（`backend/api/app/scheduler.py`），檢查有沒有預約的送出時間到了，到了就開一個無頭瀏覽器把表單真的填一填送出——這個過程完全在 VPS 上執行，跟你的手機有沒有開、在不在線上、App 有沒有被滑掉完全無關。
...
換句話說：現在你可以選好幾週後的課、完全不管手機，後端到了那天早上該送就會送，推播會通知你結果。少數會受影響的情況只剩「後端本身」的可靠度——VPS 掛了、Docker 沒開機自動啟動——這些跟 iOS 平台限制無關，是一般伺服器維運要注意的事，例如可以額外設定 `docker compose` 的 `restart: unless-stopped`（已經設定了）確保 VPS 重開機後容器會自動起來。

## Google 表單擋掉直接 POST（真實發生過、已經修好的事故）

第一次有真的預約送出時（週一 9/21 2035 TRX），App 顯示「失敗，表單回應 400」。查下去發現：

1. **先排除了資料問題**：直接讀 VPS 上 SQLite 裡存的原始資料，姓名、課程名稱都是乾淨正確的 UTF-8（不是編碼壞掉——中途一度懷疑是編碼問題，是因為我自己在 Windows 終端機上用 `curl | python -m json.tool` 查資料時，看到的亂碼其實是我自己工具鏈的顯示問題，不是資料本身的問題，繞了一圈才確認）。
2. **直接重現**：用一模一樣的資料對 Google 表單重送，一樣 400。接著試了各種組合——正確格式、故意打錯格式、補上表單要求的 `fbzx` 一次性欄位、帶瀏覽器 User-Agent、帶 cookie，甚至故意送一個完全不存在的欄位 ID——**全部都得到一模一樣的 400、一模一樣的「validation failed」錯誤標記**。連送垃圾內容都得到相同結果，代表 Google 擋的不是「內容對不對」，而是「這不是一個真的瀏覽器送出的請求」這件事本身。
3. **確認表單本身沒問題**：請使用者直接用手機瀏覽器手動填表單送出，成功看到「回應已記錄」。這排除了表單設定被關掉、或帳號被封鎖之類的可能，問題精準鎖定在「非瀏覽器的自動化 POST 被擋」。

**結論**：Google 表單現在會擋掉不執行 JavaScript 的原始 POST 請求，不管內容是什麼。這代表這個 App 從最初就依賴的「後端直接 POST 表單」這個做法已經不可行了，Google 近年一直在做這類反自動化的強化。

**修法**：把 `backend/api/app/google_form.py` 整個換掉，改用 **Playwright 開一個真的無頭 Chromium**：載入表單頁面、找到 3 個文字欄位（用 `role=textbox`，依畫面上出現的順序，不是靠 `entry.*` ID）、依序填入姓名/員工編號/課程資訊、按下「Submit」按鈕、確認頁面出現「我們已經收到您回覆的表單」之類的確認文字才算成功。這樣送出的請求跟真人用瀏覽器操作沒有分別，繞過了前面那個擋自動化的機制。

技術上顧慮的是 VPS 資源（只有 961MB 記憶體、1 顆 CPU），但實測**一個無頭瀏覽器跑完全程只用到約 160MB**，遠低於擔心的量，VPS 撐得住——而且因為有前面「6 天內最多 2 次」的安全上限擋著，真正會用到瀏覽器的頻率本來就很低，不會有多個瀏覽器實例同時佔記憶體的問題（排程迴圈本來就是一筆一筆依序處理，不會同時開兩個）。

`backend/api/tests/diagnose_browser.py` 是驗證這個機制時寫的診斷腳本，保留在 repo 裡——之後如果表單欄位順序變了、或换成正式表單，先跑這個腳本（可以用 `docker cp` 複製進 container 用 `docker exec` 跑），它會印出頁面上每個欄位的位置和送出按鈕的文字，順便存截圖到 `/tmp`，方便確認自動化邏輯要不要跟著調整。
...
1. **每人每堂課只會有一筆預約**：`create_reservation()` 一開始就檢查「這位員工」是不是已經預約過這堂課了，有的話直接回傳既有的那筆；不同員工預約同一堂課則各自一筆。另外資料庫對 `(course_id, employee_id)` 有 `UNIQUE` 限制，就算兩個請求剛好同時打進來，也只會有一筆真的寫進去。（早期版本是 `course_id` 單獨唯一＝一堂課全部人只能一筆，已用 `storage.init_db()` 的一次性遷移改掉。）
...
### 第四層：安全上限，按「員工＋日期＋課程」分開算（背景：Jofit Mark 1 的教訓）

前三層都是「保證同一筆預約最多送一次」，但沒有擋住「另一個完全不相干的 bug，讓系統一直產生新的預約並送出」這種情況——這正是 Jofit Mark 1（這個專案的前一代）實際發生過的事：某個 bug 造成每 30 秒真的送出一次，累積五千多次，報名系統整個癱瘓。既然「公司那邊完全沒有防呆機制」，這個保險就必須由我們自己擋。

做法（`reservations.py` 的 `MAX_ATTEMPTS_PER_WINDOW` / `ATTEMPT_WINDOW`）：**針對「同一位員工、同一天、同一堂課」這個組合**，不管成功或失敗，只要真的對 Google 表單發出過 POST 請求，就記一筆「嘗試」；同一個組合連續 6 天內的嘗試次數一旦達到 2 次，之後的送出一律直接擋下來（標成 `failed`，並推播通知你）；**如果其中一次已經送出成功，同一個組合就不會再嘗試第二次**，不用等到用滿 2 次。這是刻意按「員工＋日期＋課程」分開算，不是全系統共用一個額度——預約 A 課程不會影響 B 課程的額度，兩位不同員工的預約也互不影響。判斷用「已經打過幾次請求」而不是「Google 回應是不是成功」，因為 Google 表單本來就不會給我們可靠的成功訊號，能確實掌握的只有「我們是不是又發了一次請求」這件事本身。

這個視窗是**滾動式**的，不需要手動重設：6 天前那次嘗試一旦過期，額度自動空出來。但數學上，不管未來有沒有其他還沒發現的 bug，同一個「員工＋日期＋課程」組合在任何連續 6 天內對 Google 表單的真實請求次數，都保證不會超過 2 次，而且只要成功過一次就不會再送——這是不管系統其他部分是否正常，都成立的硬上限。已經寫了測試驗證（`test_reservations.py`：同一組合第 3 次嘗試會被擋、成功一次後第 2 次會被立刻擋下、不同組合彼此不受影響）。
...
圖片的部分：兩張圖是你提供的 Google Drive 連結下載下來直接放進小工具的 Assets.xcassets，兩張圖（運動圖 `ExerciseDay`、休息圖 `RestDay`）都是去背的透明 PNG，尺寸是像素圖的「原生解析度」（約 59×54、72×54，每個小方塊就是 1px），小工具用最近鄰（不平滑）以整數倍放大。原始 1408×768 的 JPEG（白底）在 git 歷史裡。之後想換圖，如果新圖是白底的像素圖，需要重新還原格線、去背後再放進來（直接放大圖會在小工具上被縮成糊的）。
...
| `ADMIN_PIN` | 管理密碼：App 的「取消所選」和「儲存課表」要帶（`X-Admin-Pin` 標頭），由後端驗證。不填＝這兩個管理動作一律拒絕。連續輸錯 5 次鎖 15 分鐘（記憶體內計數，重啟後歸零） |
...
- **真的用無頭瀏覽器成功送出過一次表單**：用 `diagnose_browser.py` 在 VPS 上實際跑過完整流程（開頁面 → 填 3 個欄位 → 按送出 → 確認出現「我們已經收到您回覆的表單」），有截圖存證，全程記憶體只用到約 160MB。這是目前唯一一次確認「送出邏輯本身真的會成功」的測試（用明顯標註「測試」字樣的假資料），其餘透過 API 的測試都因為已經觸發「6 天內最多 2 次」的安全上限而被擋下（代表擋的機制本身也如預期運作）。

## 已知未驗證事項

這個環境沒有 Mac，所有 Swift 程式碼與 CI 設定都是照標準寫法產生、未實際跑過 `xcodebuild`。第一次在 GitHub Actions 跑之前，建議先選一堂 6 天內的課測試（會馬上送出，記得用一個明顯是測試的員工編號/姓名），確認整條路徑（App → 後端 → 無頭瀏覽器 → Google 表單）真的會成功，且不會誤導負責處理報名的小編。
