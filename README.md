...
### 第四層：全域安全上限（背景：Jofit Mark 1 的教訓）
...
做法（`reservations.py` 的 `MAX_ATTEMPTS_PER_WINDOW` / `ATTEMPT_WINDOW`）：**不管是哪一筆預約、不管成功或失敗，只要真的對 Google 表單發出過 POST 請求，就記一筆「嘗試」；連續 6 天內，全部預約加起來的嘗試次數一旦達到 2 次，之後的送出一律直接擋下來**（標成 `failed`，附上「已達到 6 天內最多送出 2 次的安全上限」的訊息，並推播通知你），不會真的碰到 Google 表單。這個判斷用「已經打過幾次」而不是「Google 回應是不是成功」來算，因為 Google 表單本來就不會給我們可靠的成功訊號，能確實掌握的只有「我們是不是又發了一次請求」這件事本身。

這個視窗是**滾動式**的，不需要手動重設：6 天前那次嘗試一旦過期，額度自動空出來，日常合理使用（表單規則本來就只能約 6 天內的課）幾乎不可能碰到這個上限。但數學上，不管未來有沒有其他還沒發現的 bug，任何連續 6 天內對 Google 表單的真實請求次數，都保證不會超過 2 次——這是不管系統其他部分是否正常，都成立的硬上限。已經寫了測試驗證（`test_reservations.py` 的第三個案例：連續建立 3 筆不同課程的即時預約，確認前兩筆送出、第三筆被擋下且 Google 表單只被真的呼叫 2 次）。
