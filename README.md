...
- **預約紀錄**：排程中放最上面（送出時間相同的合併成一組），其次送出失敗，已送出的依月份摺疊；狀態用膠囊標籤（排程中＝金、已送出＝深色、失敗＝磚紅）。
...
Jofit/                       # 主 App target
  ...
  Theme.swift                 # 所有顏色（亮/暗）、圓角、字重、課程類別→色條對照、主按鈕樣式，都在這一個檔案調
...
圖片的部分：兩張圖是你提供的 Google Drive 連結下載下來直接放進小工具的 Assets.xcassets，運動圖是 1408×768 的 JPEG，休息圖（`RestDay`，今天沒課時顯示）現在也是 1408×768 的 JPEG（睡覺的小熊貓）。之後想再換圖，直接替換 `JofitWidget/Assets.xcassets/RestDay.imageset` 裡的檔案，並同步改 `Contents.json` 的檔名即可。
